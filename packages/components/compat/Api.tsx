...
export function previewImage(option: PreviewImageOption) {
    if (isTaro) return Taro.previewImage(option)
...
export function getAppBaseInfo() {
    if (isTaro) return Taro.getAppBaseInfo()
...
const themeChangeListenerMap = new Map<ThemeChangeCallback, (e: MediaQueryListEvent) => void>()
...
export function onThemeChange(callback: ThemeChangeCallback) {
    if (isTaro) return Taro.onThemeChange(callback)
...
export function offThemeChange(callback: ThemeChangeCallback) {
    if (isTaro) return Taro.offThemeChange(callback)
...
export function navigateTo(url: string) {
    if (isTaro) return Taro.navigateTo({ url: url })
